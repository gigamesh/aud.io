...
export function* loginUserSaga(action){
  ...
  try{
    ...
    console.log(err);
    yield put(actions.loginUserFail(`Status code ${err.response.status}: ${err.response.statusText}`))
  }
}
...
export function* userUpdateSaga(action){
  ...
  const response = yield axios.post(`/api/update_user`,
      action.values).then(res => res.data);
...
export function* userSignupSaga(action){
  ...
  if(!response.isAuth){
    let errorMsg;
    if(response.msg){
      errorMsg = response.msg.includes('email') ? 'Email already registered.' :
...
export function* profileDataSaga(action){
  ...
  if(id){
    try {
      const response = yield axios.get(`/api/users?id=${id}`)
        .then(res => res.data);
      ...
      console.log(err);
      yield put(actions.profileDataFail(`Status code ${err.response.status}: ${err.response.statusText}`));
    }
  }
}
